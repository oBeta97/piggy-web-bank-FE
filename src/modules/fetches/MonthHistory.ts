...
export const getMontHistory = async (page:number = 0, size:number = 10, sortBy:string = 'id'): Promise<IfetchError | Ipage<ImonthHistory>> => {
    try {
...
export const getLastMontHistory = async (): Promise<IfetchError | ImonthHistory> => {
    try {
        return await getDeleteFetch(
            import.meta.env.VITE_BACKEND_URL + `me/month-histories/last`,
            "GET"
        )
    } catch (err) {
        return {
